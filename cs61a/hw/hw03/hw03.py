...
def num_eights(pos):
    ...
    """
    "*** YOUR CODE HERE ***"
    if pos%10==8: return 1+num_eights(pos//10)
    elif pos<10:return 0
    else: return num_eights(pos//10)

# Alternate_solution
def num_eights_al(pos):
    count=0
    def eights(pos,count):
        if pos==0: return count
        if pos%10==8: return eights(pos//10,count+1)
        else: return eights(pos//10,count)
    return eights(pos,count)
...
def pingpong(n):
    ...
    def helper(x,i,step):
        if i==n: return x
        if i%8==0 or num_eights(i)>0: return helper(x+step,i+1,-step)
        else: return helper(x+step,i+1,step)
    return helper(1,1,1)

# solution for use loops
# It can make you understand better
def pingpong_while(n):
    '''x as the return value, i as the index, step is the cumulative amount'''
    x,i=1,1
    step=1
    while i<n:
...
def next_dir(step,i):
    if i%8==0 or num_eights(i)>0: return -step
    return step
